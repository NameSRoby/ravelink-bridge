...
function normalizeFixture(fixture, index) {
  ...
  if (brand === "hue") {
    return {
      ...base,
      bridgeIp: fixture.bridgeIp || process.env.HUE_BRIDGE_IP || "",
  ...
  if (brand === "wiz") {
    return {
      ...base,
      ip: fixture.ip || process.env.WIZ_BACKGROUND_IP || ""
    };
  }
...
function sanitizeFixtureForConfig(input = {}, fallbackIndex = 0, options = {}) {
  ...
  if (brand === "hue") {
    return {
      ...
      twitchEnabled,
      customEnabled,
      bridgeIp: String(input.bridgeIp || "").trim(),
      username: String(input.username || "").trim(),
      bridgeId: String(input.bridgeId || "").trim(),
      clientKey: String(input.clientKey || "").trim(),
      entertainmentAreaId: String(input.entertainmentAreaId || "").trim(),
      lightId: Math.max(1, Number(input.lightId || 1) || 1)
      }
    };
  }
