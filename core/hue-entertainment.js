...
module.exports = function createHueEntertainmentTransport({ fixtureRegistry, log = console }) {
  ...
  async function startBridgeStream(bridgeRef, areaCandidate, timeoutMs = 3500) {
    return new Promise((resolve, reject) => {
      ...
      try {
        ...
        startPromise = bridgeRef.start(areaCandidate, timeoutMs);
      } catch (err) {
        finish(err);
        return;
      }
  ...
  async function start() {
    ...
    try {
      ...
      const attemptsPrimary = [6500, 9000];
      const attemptsFallback = [8000];
      let started = false;
      ...
      for (let areaIdx = 0; areaIdx < maxAreaCandidates && !started; areaIdx += 1) {
        ...
        // Clear stale state before each area attempt.
        await stopEntertainmentArea(nextBridge, candidateArea, `start-preclear-${areaIdx + 1}`);
        await wait(220 + (areaIdx * 60));
        ...
        for (let i = 0; i < attempts.length; i += 1) {
          ...
          try {
            ...
            log.warn?.(
              `[HUE][ENT] start attempt area ${areaIdx + 1}/${maxAreaCandidates} try ${i + 1}/${attempts.length} failed: ${attemptDetail}`
            );
            await stopEntertainmentArea(nextBridge, candidateArea, `start-retry-${areaIdx + 1}-${i + 1}`);
            await wait(260 + (i * 180) + (areaIdx * 120));
          }
        }
      }
