...
module.exports = function createRaveEngine(controls) {
  ...
  const GENRE_SCENES = {
    edm:     { idle: "idle_soft", flow: "flow_edm", pulse: "pulse_strobe" },
    hiphop: { idle: "idle_soft", flow: "flow_hiphop", pulse: "flow_hiphop" },
    metal:  { idle: "idle_soft", flow: "flow_metal", pulse: "pulse_strobe" },
    ambient:{ idle: "idle_soft", flow: "flow_ambient", pulse: "flow_ambient" },
    ...
    pop:    { idle: "idle_soft", flow: "flow_pop", pulse: "flow_pop" },
    rock:   { idle: "idle_soft", flow: "flow_rock", pulse: "pulse_strobe" },
    rnb:    { idle: "idle_soft", flow: "flow_rnb", pulse: "flow_rnb" },
    techno: { idle: "idle_soft", flow: "flow_techno", pulse: "pulse_strobe" },
    media:  { idle: "idle_soft", flow: "flow_media", pulse: "flow_media" },
  ...
  const META_AUTO_GENRE_STYLE = {
    ...
    dnb: { aggression: 0.72, maxHz: 16, floorHz: 4, baseProfile: "reactive", baseReactivity: "aggressive" },
    pop: { aggression: 0.08, maxHz: 10, floorHz: 4, baseProfile: "balanced", baseReactivity: "balanced" },
    rock: { aggression: 0.4, maxHz: 12, floorHz: 4, baseProfile: "reactive", baseReactivity: "balanced" },
    rnb: { aggression: -0.18, maxHz: 8, floorHz: 2, baseProfile: "balanced", baseReactivity: "precision" },
    techno: { aggression: 0.56, maxHz: 14, floorHz: 4, baseProfile: "reactive", baseReactivity: "aggressive" },
    media: { aggression: -0.34, maxHz: 8, floorHz: 2, baseProfile: "cinematic", baseReactivity: "balanced" },
    auto: { aggression: 0.2, maxHz: 12, floorHz: 4, baseProfile: "balanced", baseReactivity: "balanced" }
  };
  ...
  function overclockLevelFromHz(hz) {
    ...
    for (let level = 7; level >= 0; level--) {
      if (safeHz >= META_AUTO_HZ_BY_LEVEL[level] - 0.25) return level;
    }
    return 0;
  }
  ...
  function computeMetaPlan(now = Date.now()) {
    ...
    const floorHz = clamp((tier <= 1 ? 2 : style.floorHz) + Math.min(0, decadeOverclockBias), 2, maxHz);
  ...
  function updateMetaAuto(now) {
    ...
    const fastPath = Boolean(plan.fastPath);
  ...
  const WIZ_SLOW_SCENES = new Set(["flow_ambient", "flow_house", "flow_pop", "flow_rnb", "flow_media"]);
  const WIZ_AGGRESSIVE_SCENES = new Set(["flow_metal", "flow_dnb", "flow_techno", "flow_rock"]);
  const WIZ_TRANCE_LIKE_SCENES = new Set(["flow_trance", "flow_edm"]);
  ...
  function getWizIntervalMs() {
    ...
    const slowMultiplier = 1 + Math.max(0.1, (1 - getEnergyDrive()) * 1.0 - motion * 0.55);
    return Math.round(base * slowMultiplier);
  }

  function applyEnergyBrightnessScale(rawBri) {
    const drive = getEnergyDrive();
    const scale = 0.12 + drive * 0.88;
    const floor = 24 + Math.round(drive * 44);
    return Math.round(clamp(rawBri * scale, floor, 254));
  }

  function getWizBrightness() {
    const drive = getEnergyDrive();
    return clamp(Math.pow(drive, 1.25), 0.06, 1);
  }
...
function pickAutoFlowScene() {
  ...
  if (audioBandMid > 0.34 || intensity > 0.5) {
    return "flow_edm";
  }
  if (audioBandHigh > 0.32 || (motion > 0.42 && audioBandHigh > 0.24)) {
    return "flow_trance";
  }
...
function getModeSwitchBias() {
  ...
  function emitHue(now, isBeat) {
    ...
    if (sceneName === "idle_soft") {
      ...
      const transitionByRate = Math.max(0, Math.round(interval / 100) - 1);
      ...
      const transitionDrop =
        ...
        (isBeat ? 1 : 0) +
        (telemetry.drop ? 1 : 0);
      transition = Math.round(clamp(baseTransition - transitionDrop, 0, scene.transition));
  ...
  function emitWiz(now, isBeat) {
    ...
    const scene = telemetry.scene || "idle_soft";
    ...
    if (flowScene) {
      ...
      const flowTextureScale = 0.62 + flowEnergy * 0.62;
      const speedBase = (0.06 + drive * 0.34 + motion * 0.62) * flowSpeedScale;
      const phraseBoost = telemetry.phrase === "build" ? 0.16 : 0;
      const beatBoost = isBeat ? (0.18 + motion * 0.24) : 0;
      const dropBoost = telemetry.drop ? 0.44 : 0;
      const intervalScale = clamp(interval / 125, 0.7, 2.8);
      ...
      const texture = (6 + motion * 24 + Math.max(0, drive - 0.15) * 18) * flowTextureScale;
      const waveR =
        Math.sin(now / 220 + wizColorCursor * 1.7 + audioBandHigh * 5.3) * texture +
        Math.sin(now / 640 + audioBandLow * 4.1) * (2 + drive * 6);
      const waveG =
        Math.sin(now / 260 + wizColorCursor * 1.25 + audioBandMid * 4.8) * texture +
        Math.sin(now / 700 + audioBandHigh * 3.9) * (2 + motion * 6);
      const waveB =
        Math.sin(now / 200 + wizColorCursor * 1.95 + audioBandLow * 4.4) * texture +
        Math.sin(now / 560 + audioBandMid * 4.2) * (2 + drive * 5);
    ...
    controls.emit({
      ...
      brightness: flowScene
        ? clamp(getWizBrightness() * clamp(0.9 + flowIntensity * 0.17, 0.7, 1.35), 0.06, 1)
        : getWizBrightness()
    });
  }
  ...
  return {
    ...
    setScene(sceneName) {
      ...
      if (normalizedLock === "flow") {
        forcedScene = resolveFlowScene(now);
      ...
      if (forcedScene) {
        ...
        console.log(
          "[RAVE] scene locked:",
          forcedSceneInput === "flow" ? `flow -> ${lockedScene} (fixed)` : lockedScene
        );
      } else {
        forcedSceneInput = null;
        sceneCandidate = stableScene;
