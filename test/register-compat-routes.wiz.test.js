// [TITLE] Test Module: test/register-compat-routes.wiz.test.js
// [TITLE] Purpose: verify WiZ discovery compatibility route behavior
...
test("compat wiz discover route returns normalized device list", async () => {
  await withCompatServer(createDeps({
    wizBridge: {
      discoverDevices: async () => ({
        ok: true,
        devices: [
          { ip: "192.168.1.80", mac: "aa:bb", moduleName: "wiz-a", roomId: 3, roomName: "Desk" },
          { ip: "bad-host", mac: "cc:dd", moduleName: "wiz-b", roomId: 4, roomName: "Wall" }
        ]
      })
    }
  }), async baseUrl => {
    const response = await requestJson(baseUrl, "GET", "/wiz/discover");
    assert.equal(response.status, 200);
    assert.equal(response.data?.ok, true);
    assert.equal(Array.isArray(response.data?.devices), true);
    assert.equal(response.data?.devices?.length, 1);
    assert.equal(response.data?.devices?.[0]?.ip, "192.168.1.80");
    assert.equal(response.data?.devices?.[0]?.roomName, "Desk");
  });
});

test("compat wiz discover route reports unavailable when adapter lane missing", async () => {
  await withCompatServer(createDeps({
    wizBridge: {}
  }), async baseUrl => {
    const response = await requestJson(baseUrl, "GET", "/wiz/discover");
    ...
    assert.equal(response.data?.error, "wiz_discovery_unavailable");
  });
});

test("compat wiz telemetry route returns adapter telemetry when available", async () => {
  await withCompatServer(createDeps({
    wizBridge: {
      discoverDevices: async () => ({ ok: true, devices: [] }),
      getTelemetry: () => ({
    ...
    const response = await requestJson(baseUrl, "GET", "/wiz/telemetry");
    assert.equal(response.status, 200);
    assert.equal(response.data?.ok, true);
    assert.equal(response.data?.source, "wiz_udp_adapter");
    assert.equal(response.data?.sent, 9);
