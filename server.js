...
const HUE_RECOVERY_MAX_COOLDOWN_MS = 45000;
...
const HUE_REST_SINGLE_FIXTURE_SAFE_MIN_MS = 150;
...
function tuneHueRestTransitionForLatency(state, effectiveRateMs = 260) {
  ...
  hueRecoveryTimer = setTimeout(async () => {
    try {
      ...
      if (result.active === HUE_TRANSPORT.ENTERTAINMENT) {
        ...
        const pendingReason = redactSensitiveLogValue(result.fallbackReason || "still on REST");
      ...
      hueRecoveryNextAt = Date.now() + cooldown;
      console.warn(`[HUE][ENT] auto-recover failed (${reason}):`, redactErrorForLog(err));
    } finally {
      hueRecoveryInFlight = false;
      hueRecoveryTimer = null;
    }
          ...
          for (const zone of zones) {
            ...
            enqueueHue(
              ...
              zone,
              {
                ...
                maxSilenceMs: hueRateProfile.maxSilenceMs,
        ...
        if (intent.type === "WIZ_PULSE") {
          ...
          for (const zone of zones) {
            enqueueWiz(
              ...
              {
                minIntervalMs: effectiveRateMs,
                maxSilenceMs: wizRateProfile.maxSilenceMs,
  ...
  try {
    ...
    const transport = await settleWithTimeout(
      ...
      () => ({
        desired: hueTransport.desired,
        active: hueTransport.active,
        fallbackReason: "entertainment switch timeout"
      })
    );
...
async function handleRaveOff(_, res) {
  ...
  hueTransport.desired = HUE_TRANSPORT.REST;
  state.unlock("rave");
...
function applyHueIntentAudioReactivity(intent = {}, telemetry = null) {
  ...
  const floorPercent = 0.52 + (profile.character * 0.14);
  const floorBri = Math.max(24, Math.min(190, Math.round(baseBri * floorPercent)));
  const boundedDrive = Math.max(0.34, Math.min(1.45, profile.drive));
  ...
  if (Number.isFinite(Number(statePatch.transitiontime))) {
    ...
    statePatch.transitiontime = Math.max(0, baseTransition - trim);
  }
...
function applyWizIntentAudioReactivity(intent = {}, telemetry = null) {
  ...
  const baseBrightness = clampNumber(next.brightness, 0.1, 1, 0.65);
  const boundedDrive = Math.max(0.34, Math.min(1.45, profile.drive));
  const floor = Math.max(0.24, Math.min(0.84, baseBrightness * (0.6 + (profile.character * 0.18))));
  ...
  next.brightness = Math.max(0.12, Math.min(1, brightness));
...
function getSystemConfigSnapshot() {
  return {
    version: Number(systemConfigRuntime?.version || 2),
